...
class V1IdentitySelfModelGetResponse(BaseModel):
    entries: list[SelfModelEntry]


class V1IdentitySelfModelReflectPostRequest(BaseModel):
    lookbackTurns: int | None = Field(
        50, description='How many recent conversation turns to consider.\n', ge=1
    )
    conversationId: UUID | None = Field(
        None,
        description='Restrict reflection to a single conversation.\nOmit to reflect across all recent conversations.\n',
    )


class V1IdentitySelfModelReflectPostResponse(BaseModel):
    entriesWritten: list[SelfModelEntry]


class V1IdentityPersonsPostRequest(BaseModel):
    displayName: str = Field(..., min_length=1)
    relationshipNote: str | None = None


class V1IdentityPersonsPersonIdPatchRequest(BaseModel):
    displayName: str | None = Field(None, min_length=1)
    relationshipNote: str | None = None


class V1IdentityLinksPendingGetResponse(BaseModel):
...
class V1IdentityPersonsGetResponse(BaseModel):
    persons: list[Person]


class V1IdentityLinksPendingLinkIdApprovePostResponse(BaseModel):
