...
The identity component is never the trust root. The watchdog generates
the per-restart HMAC signing key and distributes it via env var. This
module exposes just the decode side so route dependencies can validate
inbound bearer tokens.

Identical shape to security modules in orchestrator / hemisphere-driver
/ memory — keeping these nearly-identical across components makes the
cross-cutting contract easy to audit at a glance.
"""
