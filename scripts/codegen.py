...
`src/eugene_plexus_identity/_generated/`.
...
GENERATED_DIR = REPO_ROOT / "src" / "eugene_plexus_identity" / "_generated"
...
SPECS_TO_GENERATE = [
    ("openapi/identity.yaml", "models.py"),
...
def run_codegen(specs_root: Path, ref: str) -> None:
    ...
    for input_rel, output_name in SPECS_TO_GENERATE:
        ...
        cmd = [
            ...
            "--disable-timestamp",
            # identity.yaml has no `components.schemas` of its own — every
            # type comes from common.yaml via $ref, plus a handful of
            # inline response schemas in the paths block. Tell the
            # generator to follow paths-scope refs so the inline shapes
            # (e.g. ListPersonsResponse) end up in the output.
            "--openapi-scopes",
            "paths",
            "schemas",
        ]
